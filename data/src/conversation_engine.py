...
def initialize_chatbot(chat_store, container, username, user_info):
    ...
    shtt_engine = index.as_query_engine(
        similarity_top_k=3,
    )
    shtt_tool = QueryEngineTool(
        query_engine=shtt_engine, 
        metadata=ToolMetadata(
            name="so_huu_tri_tue",
            description=(
                "Cung cấp các thông tin liên quan đến sở hữu trí tuệ "
                "dựa theo luật sở hữu trí tuệ sửa đổi năm 2018. Sử dụng câu hỏi văn bản thuần túy chi tiết làm đầu vào cho công cụ"
            ),
        )
    )
    get_date_time_tool = FunctionTool.from_defaults(fn=get_date_time)
    agent = OpenAIAgent.from_tools(
        tools=[shtt_tool, get_date_time_tool], 
        memory=memory,
    ...
    if not os.path.exists(conversation_file) or os.path.getsize(conversation_file) == 0:
        with container:
            with st.chat_message(name="assistant", avatar=firefox_avatar):
                st.markdown("Chào bạn! Tôi là AI chuyên gia trong lĩnh vực tư vấn về sở hữu trí tuệ. Hãy cho tôi biết vấn để của bạn.")
